...
// Lấy thông tin từ sản phẩm truyền đến trang chi tiết sản phẩm 
...
// Chức năng cơ bảng của Backend...
export const createProduct = () => async (dispatch, getState) => {
    dispatch({ type: PRODUCT_CREATE_REQUEST });
    const {userSignin:{userInfo} } = getState();
    try {const { data } = await Axios.post('/api/products', {},
...
export const updateProduct = (product) => async (dispatch, getState) => {
    ...
    const {userSignin: {userInfo} } = getState();
    try {const { data } = await Axios.put(`/api/products/${product._id}`, product, 
        {headers: { Authorization: `Bearer ${userInfo.token}` }}
    );
...
export const deleteProduct = (productId) => async (dispatch, getState) => {
    ...
    const {userSignin:{userInfo} } = getState();
    try {const { data } = Axios.delete(`/api/products/${productId}`, 
        {headers: { Authorization: `Bearer ${userInfo.token}` }}
    );
